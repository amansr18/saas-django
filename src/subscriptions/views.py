...
def user_subscription_view(request):
    ...
    if request.method == "POST":
        print("refresh sub")
        finished = subs_utils.refresh_active_users_subsscription(user_ids=[request.user.id], active_only=False)
        if finished:   
            messages.success(request, "Your subscription details have been refreshed.")
        else:
            messages.error(request, "Your subscription details have not been refreshed, please try again.")
